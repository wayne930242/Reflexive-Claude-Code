...
- **新增 `refactoring-plugins` 技能**：根據官方 Claude Code 最佳實踐進行插件遷移和重構
  - 跨平台 Python 健康檢查腳本（`validate_plugin.py`，支援 `uv run`）
  - 整合官方 `claude plugin validate` CLI
  - 8 類插件健康檢查清單（manifest、結構、技能、命令、代理、路徑、版本同步、發布準備）
  - 跨技能觸發重疊偵測
  - 新增 `/refactor-plugin` 命令
