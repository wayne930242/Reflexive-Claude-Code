...
- **用戶確認 UX 改善**：技能現在要求展示完整細節後才詢問用戶確認
  - 修正「簡短摘要後直接要用戶選擇」的 anti-pattern，涵蓋 4 個技能
  - `initializing-projects`：Task 4 要求展示完整 blueprint，不可只列摘要
  - `planning-agent-systems`：Task 5 要求展示每個元件的詳細計畫與理由
  - `analyzing-agent-systems`：Task 4 要求展示完整分析結果與影響說明
  - `writing-skills`：Task 1 要求展示完整需求分析後才繼續
  - 新增 anti-pattern 警告與 rationalization 條目，防止 agent 給予不足的上下文
