...
- **核心 chain 加入 review 步驟**：將 review 從 refactor 拆分為獨立步驟
  - 新增 `reviewing-agent-systems` 技能：統整呼叫全部 5 個 reviewer agents
  - 新增 `hook-reviewer` 代理：審查 hook exit codes、效能、註冊設定
  - 新增 `subagent-reviewer` 代理：審查單一職責、工具最小化、模型選擇
  - 核心 chain 變為：analyze → brainstorm → plan → apply → **review** → refactor
  - `writing-hooks` 和 `writing-subagents` 新增 REFACTOR 品質審查步驟
  - `reflecting` 現在會對新整合的元件執行 reviewer agents
  - `refactoring-agent-systems` 改為接收 review 報告，不再自行分析
