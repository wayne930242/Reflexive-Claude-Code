...
- **Agent system 重構**：系統性清理所有元件
  - 移除 rcc-dev 插件中重複的 `creating-plugins` 技能
  - 為 3 個 reviewer agents 加入 `context: fork` 隔離
  - 4 個超標技能透過漸進式揭露降至 300 行以下，內容移至 `references/`
  - 區分 `improving-skills`（單一技能）與 `refactoring-skills`（多技能）的觸發條件
  - 移除 CLAUDE.md 中過時的版本引用
